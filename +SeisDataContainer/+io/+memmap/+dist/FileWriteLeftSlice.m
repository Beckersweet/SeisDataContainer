...
% Read file
if header.distributedIO
    SeisDataContainer.io.memmap.dist.DataWriteLeftSlice(1,header.directories,'real',real(x),...
        header.size,header.distribution,slice,header.precision);
    if header.complex
        SeisDataContainer.io.memmap.dist.DataWriteLeftSlice(1,header.directories,'imag',imag(x),...
            header.size,header.distribution,slice,header.precision);
    end
else
    SeisDataContainer.io.memmap.dist.DataWriteLeftSlice(0,dirname,'real',real(x),...
        header.size,header.distribution,slice,header.precision);
    if header.complex
        SeisDataContainer.io.memmap.dist.DataWriteLeftSlice(0,dirname,'imag',imag(x),...
            header.size,header.distribution,slice,header.precision);
    end
end
