...
% Write file
if distribute
    SeisDataContainer.io.memmap.dist.DataAlloc(header.directories,'real',...
        header.distribution.size,f_precision);
    SeisDataContainer.io.memmap.dist.DataWrite(1,header.directories,'real',...
        real(x),header.distribution,f_precision);
    if ~isreal(x)
        SeisDataContainer.io.memmap.dist.DataAlloc(header.directories,'imag',...
            header.distribution.size,f_precision);
        SeisDataContainer.io.memmap.dist.DataWrite(1,header.directories,'imag',...
            imag(x),header.distribution,f_precision);
    end
else
    SeisDataContainer.io.memmap.serial.DataAlloc(dirname,'real',...
        size(x),f_precision);
    SeisDataContainer.io.memmap.dist.DataWrite(0,dirname,'real',...
        real(x),header.distribution,f_precision);
    if ~isreal(x)
        SeisDataContainer.io.memmap.serial.DataAlloc(dirname,'imag',...
            size(x),f_precision);
        SeisDataContainer.io.memmap.dist.DataWrite(0,dirname,'imag',...
            imag(x),header.distribution,f_precision);
    end
end
% Write header
