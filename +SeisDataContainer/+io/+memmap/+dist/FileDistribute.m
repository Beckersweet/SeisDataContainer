...
% Read header
hdrin = SeisDataContainer.io.memmap.serial.HeaderRead(dirin);
...
% Allocate file
SeisDataContainer.io.memmap.dist.DataAlloc(hdrout.directories,'real',...
    hdrout.distribution.size,hdrout.precision);
if hdrin.complex
    SeisDataContainer.io.memmap.dist.DataAlloc(hdrout.directories,'imag',...
        hdrout.distribution.size,hdrout.precision);
end
% Copy file
for s=1:prod(sldims)
    slice = SeisDataContainer.utils.getSliceIndexS2V(sldims,s);
    x=SeisDataContainer.io.memmap.dist.DataReadLeftSlice(0,dirin,'real',...
        hdrin.size,hdrin.distribution,slice,hdrin.precision,hdrin.precision);
    SeisDataContainer.io.memmap.dist.DataWriteLeftSlice(1,hdrout.directories,'real',x,...
        hdrout.size,hdrout.distribution,slice,hdrout.precision);
    if hdrin.complex
        x=SeisDataContainer.io.memmap.dist.DataReadLeftSlice(0,dirin,'imag',...
            hdrin.size,hdrin.distribution,slice,hdrin.precision,hdrin.precision);
        SeisDataContainer.io.memmap.dist.DataWriteLeftSlice(1,hdrout.directories,'imag',x,...
            hdrout.size,hdrout.distribution,slice,hdrout.precision);
    end
end
%Write header
SeisDataContainer.io.memmap.serial.HeaderWrite(dirout,hdrout);
