...
% Allocate file
SeisDataContainer.io.memmap.serial.DataAlloc(dirout,'real',...
    hdrout.size,hdrout.precision);
if hdrin.complex
    SeisDataContainer.io.memmap.serial.DataAlloc(dirout,'imag',...
        hdrout.size,hdrout.precision);
end
% Copy file
for s=1:prod(sldims)
    slice = SeisDataContainer.utils.getSliceIndexS2V(sldims,s);
    x=SeisDataContainer.io.memmap.dist.DataReadLeftSlice(1,hdrin.directories,'real',...
        hdrin.size,hdrin.distribution,slice,hdrin.precision,hdrin.precision);
    SeisDataContainer.io.memmap.dist.DataWriteLeftSlice(0,dirout,'real',x,...
        hdrout.size,hdrout.distribution,slice,hdrout.precision);
    if hdrin.complex
        x=SeisDataContainer.io.memmap.dist.DataReadLeftSlice(1,hdrin.directories,'imag',...
            hdrin.size,hdrin.distribution,slice,hdrin.precision,hdrin.precision);
        SeisDataContainer.io.memmap.dist.DataWriteLeftSlice(0,dirout,'imag',x,...
            hdrout.size,hdrout.distribution,slice,hdrout.precision);
    end
end
%Write header
