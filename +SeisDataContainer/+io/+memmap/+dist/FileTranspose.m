...
% Allocate file
SeisDataContainer.io.memmap.dist.DataAlloc(headerOut.directories,'real',...
    headerOut.distribution.size,headerOut.precision);
if headerOut.complex
    SeisDataContainer.io.memmap.dist.DataAlloc(headerOut.directories,'imag',...
        headerOut.distribution.size,headerOut.precision);
end

% Transpose file
SeisDataContainer.io.memmap.dist.DataTranspose(dirnameIn,dirnameOut,'real',...
    dim2D,sepDim,headerOut.precision);
if headerOut.complex
    SeisDataContainer.io.memmap.dist.DataTranspose(dirnameIn,dirnameOut,...
