...
% Read file
if header.distributedIO
    x=SeisDataContainer.io.memmap.dist.DataReadLeftSlice(1,header.directories,'real',...
        header.size,header.distribution,slice,header.precision,x_precision);
    if header.complex
        dummy=SeisDataContainer.io.memmap.dist.DataReadLeftSlice(1,header.directories,'imag',...
            header.size,header.distribution,slice,header.precision,x_precision);
        x=complex(x,dummy);
    end
else
    x=SeisDataContainer.io.memmap.dist.DataReadLeftSlice(0,dirname,'real',...
        header.size,header.distribution,slice,header.precision,x_precision);
    if header.complex
        dummy=SeisDataContainer.io.memmap.dist.DataReadLeftSlice(0,dirname,'imag',...
            header.size,header.distribution,slice,header.precision,x_precision);
        x=complex(x,dummy);
    end
end
