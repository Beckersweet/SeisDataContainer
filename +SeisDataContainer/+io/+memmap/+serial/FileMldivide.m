...
SeisDataContainer.io.memmap.serial.FileAlloc(dirnameOut,headerY);

for i=1:prod(sizeB(sepDim:end))
    slice = SeisDataContainer.io.memmap.serial.FileReadLeftSlice(B,i);
    slice = reshape(slice,sizeA(2),numel(slice)/sizeA(2));
    prodz = mldivide(double(A),slice);
    SeisDataContainer.io.memmap.serial.FileWriteLeftSlice...
        (dirnameOut,prodz,i);
end
end
