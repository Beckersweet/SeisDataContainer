...
% Read header
headerIn = SeisDataContainer.io.memmap.serial.HeaderRead(dirnameIn);
...
% Transpose file
SeisDataContainer.io.memmap.serial.DataAlloc(dirnameOut,'real',headerOut.size,headerOut.precision);
SeisDataContainer.io.memmap.serial.DataTranspose(dirnameIn,dirnameOut,'real',...
    dim2D,headerOut.precision);
if headerOut.complex
    SeisDataContainer.io.memmap.serial.DataAlloc(dirnameOut,'imag',headerOut.size,headerOut.precision);
    SeisDataContainer.io.memmap.serial.DataTranspose(dirnameIn,dirnameOut,'imag',...
        dim2D,headerOut.precision);
end
end
