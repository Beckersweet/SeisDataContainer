...
% Write file
SeisDataContainer.io.NativeBin.serial.DataWriteLeftSlice(dirname,'real',real(x),...
    header.size,slice,header.precision);
if ~isreal(x)
    SeisDataContainer.io.NativeBin.serial.DataWriteLeftSlice(dirname,'imag',imag(x),...
        header.size,slice,header.precision);
end
