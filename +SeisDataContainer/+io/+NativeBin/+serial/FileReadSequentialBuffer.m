...
assert(SeisDataContainer.io.isFileClean(dirname));
...
% Read file
x=SeisDataContainer.io.NativeBin.serial.DataReadLeftChunk(dirname,'real',...
    dims,range,[],header.precision,x_precision);
if header.complex
    dummy=SeisDataContainer.io.NativeBin.serial.DataReadLeftChunk(dirname,'imag',...
        dims,range,[],header.precision,x_precision);
    x=complex(x,dummy);
end 
end
