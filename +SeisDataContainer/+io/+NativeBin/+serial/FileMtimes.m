...
SeisDataContainer.io.NativeBin.serial.FileAlloc(dirnameOut,headerY);

for i=1:prod(sizeB(sepDim:end))
    slice = SeisDataContainer.io.NativeBin.serial.FileReadLeftSlice(B,i);
    slice = reshape(slice,sizeA(2),numel(slice)/sizeA(2));
    prodz = mtimes(double(A),slice);
    SeisDataContainer.io.NativeBin.serial.FileWriteLeftSlice...
        (dirnameOut,prodz,i);
end
end