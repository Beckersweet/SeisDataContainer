...
% Write file
SeisDataContainer.io.NativeBin.serial.DataWriteLeftChunk(dirname,'real',real(x),...
    header.size,range,slice,header.precision);
if ~isreal(x)
    SeisDataContainer.io.NativeBin.serial.DataWriteLeftChunk(dirname,'imag',imag(x),...
        header.size,range,slice,header.precision);
end
