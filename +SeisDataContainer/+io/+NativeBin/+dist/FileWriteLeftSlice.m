...
partition = SeisDataContainer.utils.defaultDistribution(header.size(distdim));
if ~header.distributedIO
    header = SeisDataContainer.addDistHeaderStruct(header,distdim,partition);
else
...
% Read file
if header.distributedIO
    SeisDataContainer.io.NativeBin.dist.DataWriteLeftSlice(1,header.directories,'real',real(x),...
        header.size,header.distribution,slice,header.precision);
    if header.complex
        SeisDataContainer.io.NativeBin.dist.DataWriteLeftSlice(1,header.directories,'imag',imag(x),...
            header.size,header.distribution,slice,header.precision);
    end
else
    SeisDataContainer.io.NativeBin.dist.DataWriteLeftSlice(0,dirname,'real',real(x),...
        header.size,header.distribution,slice,header.precision);
    if header.complex
        SeisDataContainer.io.NativeBin.dist.DataWriteLeftSlice(0,dirname,'imag',imag(x),...
            header.size,header.distribution,slice,header.precision);
    end
end
