...
spmd
    ...
    if distribute
        SeisDataContainer.io.NativeBin.serial.DataWrite(dirname,filename,lx,file_precision);
    else
        SeisDataContainer.io.acquireIOlock(dirname);
        SeisDataContainer.io.NativeBin.serial.DataWriteLeftChunk(dirname,filename,lx,...
            size(x),cindx_rng,[],file_precision);
        SeisDataContainer.io.releaseIOlock(dirname);
    end
end
