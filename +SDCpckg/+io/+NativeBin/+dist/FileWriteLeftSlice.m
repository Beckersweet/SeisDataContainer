...
% Read file
if header.distributedIO
    SDCpckg.io.NativeBin.dist.DataWriteLeftSlice(1,header.directories,'real',real(x),...
        header.size,header.distribution,slice,header.precision);
    if header.complex
        SDCpckg.io.NativeBin.dist.DataWriteLeftSlice(1,header.directories,'imag',imag(x),...
            header.size,header.distribution,slice,header.precision);
    end
else
