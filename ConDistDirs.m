...
%
    ...
    methods ( Access = public )
        function td = ConDistDirs(varargin)
            ...
            if nargin > 1
                ...
                % could be exist(td)
                tdHome = td.Home;
                tdPaths = DataContainer.utils.Cell2Composite(td.Paths);
                spmd
                    assert(isdir(tdHome));
                    assert(isdir(tdPaths));
                end
            else
                [td.Paths td.Home] = DataContainer.io.makeDistDir(varargin{:});
            end
    ...
    methods ( Access = private )
        ...
        if ~td.Keep
            ...
            DataContainer.io.deleteDistDir(td.Paths)
